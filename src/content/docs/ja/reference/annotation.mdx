...
- `IdType.ASSIGN_ID`：ID を自動割り当てします。`Long`、`Integer`、`String` 型のプライマリキーに適用されます。デフォルトでは `IdentifierGenerator` の `nextId` を通じてスノーフレークアルゴリズムを使用して実装されます。<Badge text="@since 3.3.0"/>
...
:::note
この機能を使用するには、[自動フィールド補完](/guides/auto-fill-field)機能と併せて使用する必要があります。
:::
...
**注意点**：カスタム型ハンドラーを有効にするためには、それが MyBatis-Plus の設定で正しく登録され、実行時にロードできることを確認する必要があります。また、カスタム型ハンドラーの使用は慎重に行い、その正確性とパフォーマンスを確保する必要があります。詳細については、[型ハンドラー](/guides/type-handler)の内容を参照してください。
...
プラグインの使用方法の詳細については[プラグインメイン](/plugins)を参照してください。
